...
public class JbpmCoreImp implements JbpmCore {
	...
	public long addProcessInstance(String processName, Document document,
		...
		instance.getContextInstance().setVariable("docTitle",
				document.getTitle());
		...
		if (props != null) {
			for (DocumentProperty property : props) {
				if (!property.isNull()) {
					...
					instance.getContextInstance().setVariable(propertyname,
							value);
				}
			}
		}
	...
	public void delProcessDefinition(String processName) {
		JbpmContext context = getJbpmContext();
		List defs = context.getGraphSession().findAllProcessDefinitionVersions(
				processName);
	...
	public String deployProcessDefinition(byte[] processDef) {
		...
		ProcessDefinition def = ProcessDefinition
				.parseXmlInputStream(new ByteArrayInputStream(processDef));
		context.deployProcessDefinition(def);
		context.close();
		return def.getName();
	}

	public String deployProcessDefinition(String processdefintion)
			throws FileNotFoundException {
		System.err.println("jbpmCoreImp");
		JbpmContext context = getJbpmContext();
		ProcessDefinition processDefinition = ProcessDefinition
				.parseXmlInputStream(new FileInputStream(new File(
						processdefintion)));
		if (context == null) {
	...
	public String nextStep(long processInstanceId, String actorId,
		...
		if (startNodeName.equals(currentNodeName)) {
			...
			List taskInstances = context.getTaskMgmtSession()
	...
	public Object[] backStep(long processInstanceId, String actorId) {
		...
		// 搜索用户对应的所有的任务实例
		List taskInstances = context.getTaskMgmtSession().findTaskInstances(
				actorId);
			...
			if (ti.getProcessInstance().getId() == processInstanceId) {
				...
				for (Iterator iterator2 = set.iterator(); iterator2.hasNext();) {
					...
					if (t.getFrom().equals(
						...
						// 重新创建流程实例对象
						ProcessInstance pi = new ProcessInstance(ti
	...
	public List searchNextTransitions(long processInstanceId, String actorId) {
		...
		ProcessInstance instance = context
				.getProcessInstance(processInstanceId);
		System.out.println("JbpmCoreImp searchNextTransitions  "
				+ processInstanceId);
	...
	private JbpmContext getJbpmContext() {
		JbpmContext context = jbpmTemplate.getJbpmConfiguration()
				.createJbpmContext();
		return context;
	}
