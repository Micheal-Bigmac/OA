...
!function($) {
    ...
    $.fn.valid=function(object,options){
        ...
        if(myoptions !=null && validationError){
           ...
           /* myobject.append('<span id="validerrmsg" class="help-block" style="color: #FF0000;">'+myoptions+'</span>');*/
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'float', validate: function(value) {return (!/^\d+(.\d+)?$/.test(value));}, defaultMsg: '请输入数值。'},
            {name: 'image', validate: function(value) {return (!/.*.[jpg|png|bmp]$/.test(value));}, defaultMsg: '请选择jpg png bmp。'},
            {name: 'xml', validate: function(value) {return (!/.*.xml$/.test(value));}, defaultMsg: '请选择xml。'},
            {name: 'doc', validate: function(value) {return (!/.*.[doc|docx]$/.test(value));}, defaultMsg: '请选择word 文档。'},
           ...
           /* {name: 'char', validate: function(value) {return (!/^[a-z\_\-A-Z]*$/.test(value));}, defaultMsg: '请输入英文字符。'},*/
            ...
            {name: 'date',validate:function(value){return(/Invalid|NaN/.test(new Date(value).toString()));},defaultMsg:"日期格式 yyyy-MM-dd"}
        ],
        reqmark:true,
    ...
    var validateField = function(field, valid) { 
            ...
            for (j = 0; j < rules.length; j++) {
                ...
                if (flag == rule.name) {
                    ...
                    if (rule.validate.call(field, value) == x) {
                        error = true;
                        if (el.attr('type')!=null && el.attr('type').toLowerCase()=='s-file'){
        ...
        if(form){
            ...
            if(fstyle == 0){
                ...
                el.append('<span class="help-block" id="valierr">' + errorMsg +'</span>');
            }
            else if(fstyle == 1){
                ...
                controlGroup.find("#valierr").remove();
                el.parent().append('<span class="help-block" id="valierr">' + errorMsg +'</span>');
            }
        };//end !form
        return !error;
    };
        ...
        if (globalOptions.reqmark==true){
            if(fform_style==0){
                $(obj).find(".control-group>label").each(function(){
                    ...
                    el.append('<span id="autoreqmark" style="color:#FF9966"> *</span>')
                });
            }
                ...
                $(obj).find('input, textarea').each(function(){
                    ...
                    if (valid){
                        if ($.inArray('required',valid)>=0){
                            el.parent().append('<span class="help-block" id="valierr" style="color:#FF9966">*</span>');
                        }
                    };
                }); 
            };
