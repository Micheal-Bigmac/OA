			...
			<div class="span6">
				<div class="control-group">
					<div class="controls">
						...
						<option value="recordUser">按录入人</option>
						<option value="name">按考核名称</option>
				</select>
...
<div class="row-fluid">
	<form>
		<table>
			<div class="row-fluid">
				<span class="newfont07">选择：</span> <a href="PerformanceExamineAction!getAddData" class="btn ajaxify">添加绩效考核</a>
			</div>
			<div class="row-fluid">
				<div class="row-fluid" style="text-align: center; font-size: 20px; background-color: #EEEEEE">考核参数列表</div>
				<table class="table table-striped">
					...
					<tbody>
						<s:iterator value="#request.listObject" var="pe">
							<tr bgcolor="#FFFFFF">
								<td>${pe.id}</td>
								<td><a class="ajaxify" href="PerformanceExamineAction!getPerformanceExamineInfo?peId=${pe.id}">${pe.name}</a></td>
								<td>${pe.instruction}</td>
								<td>${pe.recordUser}</td>
								<td>${pe.date}</td>
								<td><a class="deleteOne" href="javascript:void(0)" data-action="PerformanceExamineAction!deletePerformanceExamine?peId=${pe.id}&method=8|${url}">删除</a></td>
							</tr>
						</s:iterator>
					</tbody>
				</table>
