	...
	<div class="portlet box green">
		...
		<div class="portlet-body form">
			...
			<form id="form" action="PerformanceExamineAction!addPerformanceExamine?method=1" class="form-horizontal" method="post">
				...
				<div class="row-fluid">
					<div class="span6 ">
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" class="m-wrap span12" placeholder="" name="listPerformanceExamine.name" value="${listPerformanceExamine.name}" check-type="required"> <span class="help-block"></span>
							</div>
						</div>
					</div>
					<div class="span6">
				...
				<table>
					<div class="row-fluid">
						<div class="row-fluid" style="text-align: center;font-size:20px;background-color:#EEEEEE">请打分</div>
							...
							<thead>
								<tr>
									...
									<s:iterator value="#request.listParameters" var="param">
										<th><s:property value="#param.name" />
										</th>
									</s:iterator>
								</tr>
							</thead>
							<tbody>
								...
								<s:iterator value="#request.listPersons" var="per" status="status">
									<tr>
										...
										<td><input type="hidden" name="pername" value="<s:property value="#per.name" />"> <s:property value="#per.name" />
										</td>
										<s:iterator value="#request.listParameters" var="pcn" status="sta">
											<td><input type="hidden" name="props[${i}].personId.id" value="${per.id}" /> <input type="hidden" name="props[${i}].paramId.id" value="${pcn.id}" />
												<div class="control-group">
													<input type="text" class="span${item }" placeholder="" name="props[${i}].score" check-type="number" /> <span class="help-block"></span>
												</div> <s:set var="i" value="#i+1" /></td>
										</s:iterator>
									</tr>
								</s:iterator>
							</tbody>
