			...
			<div class="span6">
				<div class="control-group">
					<div class="controls">
						...
						<option value="name">按流程名称</option>
				</select>
						<td><input id="className2" name="className2" type="hidden" value="2" /> 
				<input id="className" name="className" type="hidden" value="WorkFlow" /> 
				<input id="textfield" name="textfield"	type="text" /></td>
					</div>
				</div>
			</div>
			<div class="span6">
...
<div class="row-fluid">
	<form id="fom">
		<table>
			<div class="row-fluid">
				<span class="newfont07">选择：<a href="#" id="selectAll">全选</a>-<a href="#" id="unselect">反选</a> <%--<input class="btn" name="Submit" type="button" 	value="删除所选信息" id="deleteChose" data-action="DynamicFormAction!deleteDynamicForm|${url }a=1&method=8" />--%>
				</span>
			</div>
			<div class="row-fluid">
				<div class="row-fluid" style="text-align: center; font-size: 20px; background-color: #EEEEEE">表单定义列表</div>
				<table class="table table-striped">
					...
					<tbody>
						<s:iterator var="workflow" value="#request.listObject">
							<tr>
								<td><s:if test="#workflow.form != null">
								...
								<td><a class="ajaxify" href="WorkFlowAction!edit?workFlow.id=${workflow.id}">${workflow.name} |<s:property value="#workflow.processDefinition.lastIndexOf('\\\\')" /></a></td>
								<s:set var="num" value="#workflow.processDefinition.lastIndexOf('\\\\')" />
								<td><s:set var="process" value="#workflow.processDefinition.substring((#num+1))" /> <a class="ajaxify"
									href="javascript:window.showModalDialog('upload/${process}','','dialogWidth=800px;dialogHeight=500px;location=0,menubar=0,resizable=0,scrollbars=0,status=0,toolbar=0;');"> ${process }</a></td>
								<s:set var="number" value="#workflow.processDefinition.lastIndexOf('\\\\')" />
								<td><s:set var="image" value="#workflow.processImage.substring((#num+1))" /> <a class="ajaxify"
									href="javascript:window.showModalDialog('upload/${image}','','dialogWidth=800px;dialogHeight=500px;location=0,menubar=0,resizable=0,scrollbars=0,status=0,toolbar=0;');"> ${image } </a></td>
								<td><a class="ajaxify" href="DynamicFormAction!listFormField?workflowid=${workflow.id}">定义表单</a> <s:if test="#workflow.form != null">
