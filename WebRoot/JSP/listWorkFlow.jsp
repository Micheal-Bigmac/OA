...
<div class="row-fluid">
	<form name="fom" id="fom" method="post" action="">
		<table class="table table-bordered">
			<div class="row-fluid">
				<span >选择：<a href="javascript:void(0)" id="selectAll">全选</a>-<a href="javascript:;" id="unselect">反选</a>
				</span>
				<button class="btn" id="deleteChose" data-action="WorkFlowAction!deleteWorkFlow|${url }&method=8">删除所选信息</button>
				<a class="ajaxify btn" href="JSP/submitDocument.jsp" >添加流程信息</a>
			</div>
			<div class="row-fluid">
				<div class="row-fluid" style="text-align: center; font-size: 20px; background-color: #EEEEEE">流程详细列表</div>
					...
					<tbody>
						<s:iterator var="workflow" value="#request.listObject">
							<tr>
								...
								<td><a class="ajaxify" href="WorkFlowAction!edit?workFlow.id=${workflow.id }">${workflow.id	}</a></td>
								<td><a class="ajaxify" href="WorkFlowAction!edit?workFlow.id=${workflow.id}">${workflow.name}</a></td>
								...
								<td>
									...
									<s:set var="process" value="#workflow.processDefinition.substring((#num+1))" />
									 <a class="ajaxify" href="javascript:window.showModalDialog('upload/${process}','','dialogWidth=800px;dialogHeight=500px;location=0,menubar=0,resizable=0,scrollbars=0,status=0,toolbar=0;');"> ${process }</a>
								</td>
									
								<td>
									<s:set var="number" value="#workflow.processDefinition.lastIndexOf('\\\\')" />
									<s:set var="image" value="#workflow.processImage.substring((#num+1))" />
									<a class="ajaxify" href="javascript:window.showModalDialog('upload/${image}','','dialogWidth=800px;dialogHeight=500px;location=0,menubar=0,resizable=0,scrollbars=0,status=0,toolbar=0;');"> ${image } </a>
								</td>
								<td>
									<a class="ajaxify" href="JSP/submitDocument.jsp?id=${workflow.id}&method=4">编辑(修改)</a> 
									<a class="deleteOne" href="javascript:void(0)" data-action="WorkFlowAction!deleteWorkFlow?delid=${workflow.id }&method=8|${url}index=${requestScope.currentIndex-1}">删除</a>
								</td>
							</tr>
						</s:iterator>
					</tbody>
